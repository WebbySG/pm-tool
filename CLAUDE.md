...
- **Existing SEO projects WERE backfilled 2026-08-19** (owner asked for the tabs to hold the tasks): [scripts/backfill-seo-setup-tasks.mjs](scripts/backfill-seo-setup-tasks.mjs) created **56 tasks across all 14 live seo/both projects** — 11 assigned off their weekly SEO plan, 1 off project staffing, 2 left unassigned. The script is idempotent (skips any project that already holds a `seo_phase` task), reads its titles/descriptions out of `lib/seo-setup.ts` so a backfilled project matches an app-created one exactly, and defaults to a dry run (`--apply` to write). Undo, before anyone edits them: `delete from pm_tasks where seo_phase is not null;`.
