...
- **A quote IS a `pm_invoices` row** with `doc_type='quote'`. It reuses everything — line items, discounts, the PDF, templates, the editor — so there is no separate quotes table/page. **Requires the [scripts/quotes-schema.sql](scripts/quotes-schema.sql) migration** to be run once in the LIVE Supabase project (`tfhzuruaaymfhqmeiusr`, via the SQL editor — NOT the MCP). Until it's run, creating a quote fails (no `doc_type` column / `next_quote_number` RPC).
