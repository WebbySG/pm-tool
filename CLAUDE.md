...
| `pm_seo_checklist_items` | `id`, `project_id` (FK → `pm_projects`, ON DELETE **CASCADE**), `category` (`technical`/`onpage`, **unconstrained text** — see the module), `label`, `sort_order`, `done`, `note`, `completed_by` (auth uid), `completed_at`, `created_by` (DB `DEFAULT auth.uid()`), `created_at`, `updated_at` (maintained by BEFORE-UPDATE trigger `pm_seo_checklist_items_touch`). **Per-project Technical / On-Page SEO checklist record.** RLS `pm_allow_all` (delivery data, not money). Migration [scripts/seo-checklist-schema.sql](scripts/seo-checklist-schema.sql), **applied live 2026-08-18**. |
| `pm_keywords` | `id`, `project_id` (FK → `pm_projects`, ON DELETE **CASCADE**), `keyword`, `search_volume`, `difficulty`, `target_url`, `current_rank` (NULL = not ranking / never checked), `rank_checked_at`, `status` (`target`/`in_progress`/`ranking`/`dropped`) + `priority` (`high`/`medium`/`low`) — both **unconstrained text**, `notes`, `sort_order`, `created_by` (DB `DEFAULT auth.uid()`), `created_at`, `updated_at` (BEFORE-UPDATE trigger `pm_keywords_touch`). **Per-project keyword research.** Index on `(project_id, lower(keyword))` for the importer's dedupe — deliberately NOT unique. RLS `pm_allow_all` (delivery data, not money). Migration [scripts/keywords-schema.sql](scripts/keywords-schema.sql), **applied live 2026-08-18**. |
...
- **Open/active state** (not done): counts as an active task everywhere; the status cascade still overwrites it when a parent changes (since 2026-08-19 that means ANY parent status, not just approval).
...
- **Cascade/roll-up:** ⚠️ its old protection is GONE — until 2026-08-19 `cascadeDescendantsClosed` never force-dones a `pending_article_post` descendant (closing it skips the link), but the generalised status cascade has no protected statuses, so **closing a parent now closes an awaiting-link child and its `article_url` is never recorded**. Record the link first. `rollupTopStatus` still treats a top parked in `pending_article_post` like `pending_client_approval` (descendant pending_review doesn't clobber it; descendant revision_required still overrides).
...
- **Display entries** (color `#f97316` orange) in: task-drawer `statusOptions`, kanban `STATUS_COLS` (own column), tasks-page `statusConfig` + status filter + "Pending Article Post" group + passive row circle + **"Article Post" quick-filter tab** (2026-08-18; Overdue/Today/Upcoming date groups — the wait is on STAFF — and it lists the weekly-SEO article SUBTASKS via `surfacedDescendants`), activity `STATUS_LABEL`, chat `taskStatusColor/Label` + side-panel grouping, schedule `STATUS_COLORS`. The recorded `article_url` renders as a green **Article Link** cell in the drawer meta grid (permanent record).
...
- **Cascade:** rejecting a task cascades `rejected` onto **every** descendant (`cascadeDescendantsStatus`; since 2026-08-19 done/missed/pending_article_post descendants are no longer exempt). **Roll-up:** a rejected top is a terminal admin decision — descendant `pending_review` AND `revision_required` both leave it alone (`rollupTopStatus`).
...
- **Tasks-page tab (2026-08-02):** the All Tasks tab switcher has a **"To Be Discussed" quick-filter tab** (cyan, count badge) — it shares the single-status tab body (`activeTab === "discuss"` branch) but renders ONE FIFO group sorted oldest-parked-first by `statusChangedAt` instead of the Overdue/Today/Upcoming date groups (parked tasks are admin-parked, never overdue-flagged). Member/type filter dropdowns apply as on the other single-status tabs. See the All Tasks tab switcher note for the full tab set.
- **Non-rolling-up SUBTASKS surface as their own rows (2026-08-02, generalised 2026-08-18).** Only `pending_review` and `revision_required` roll up onto the top-level task (`rollupTopStatus`), so a child in any other admin-set state is invisible in the tasks page's top-level lists (`allTasks` maps `p.tasks` only). First hit as a user report for `to_be_discussed` (parked "Monday article" child didn't appear; clicking showed only the parent). **`surfacedDescendants` + `surfacedOf(status)`** in [app/(app)/tasks/page.tsx](app/(app)/tasks/page.tsx) (was `parkedDescendants`, `to_be_discussed`-only) walks every project's task tree and adds each descendant whose status is in **`SURFACED_CHILD_STATUSES`** — `to_be_discussed`, `pending_client_approval`, `pending_article_post` — as a row (with a `parentTitle` " › parent" context in the row's project line) to that status's main-tab group (`withSurfaced`), its quick-filter tab, and its status-dropdown count. **`pending_article_post` is the case that bites hardest:** the weekly SEO engine creates Article 1/2/3 as SUBTASKS, so admin approval parks the "waiting to be posted" work on a child — without this, the Article Post tab would be empty in exactly the case it exists for. `liveSelectedTask` uses a deep `findDeep` search so clicking such a row opens the CHILD's drawer, not the parent's. **If another status ever stops rolling up, add it to `SURFACED_CHILD_STATUSES`.**

### Review Workflow Roll-up & Status Cascade (2026-07-23, cascade generalised 2026-08-19)

Children/grandchildren at ANY depth have the full submit-for-review / approve / request-revision workflow (each drawer child panel carries the same footer controls). Two automatic rules in [lib/store.ts](lib/store.ts) (`rollupTopStatus`, `cascadeDescendantsStatus`, wired into `updateTaskStatus`/`updateSubtaskStatus`/`updateSubtask`/`requestTaskApproval`/`approveTaskCompletion`/`markArticlePosted`/`rejectTask`/`markTaskRejected`):
...
- **Status cascade (parent → ALL descendants) — user rule 2026-08-19: "whenever i apply a status change to parent task, apply it to all task".** `cascadeDescendantsStatus` writes the parent's NEW status onto **every** descendant at any depth, locally + in one bulk `dbUpdateTasksBulk`. This replaced the old pair of cascades (`cascadeDescendantsClosed` for done/rejected, `cascadeDescendantsRevision` for revision_required) — **there is now ONE rule for every status**, so a parent can never claim something different from the work under it.
  - **No protected statuses.** The old completion cascade skipped `done`/`missed`/`rejected`/`pending_article_post` descendants (closing a parent must not discard a record); the revision cascade already ignored that protection. The generalised rule follows revision: a done, missed, rejected or awaiting-article-link child **does** follow its parent, because changing the parent IS the instruction. Only descendants already in the target status are skipped (nothing to write). **Consequence to know:** approving/closing a parent whose child sits in `pending_article_post` now closes that child too, so the live article link never gets recorded on it — record the link before closing the parent.
  - **Wired into every status write path** so it can't fire inconsistently: `updateTaskStatus` (drawer dropdown, kanban drag, tasks-page quick-done, schedule tick, archive Reopen), `updateSubtaskStatus` (drawer subtask checkbox/cycle), `updateSubtask` (generic patch carrying a status), `requestTaskApproval` (**Submit for Review on a parent submits the whole branch**), `approveTaskCompletion` (both the `done` and the `pending_article_post` branch), `markArticlePosted`, `rejectTask` (Request Revision) and `markTaskRejected`.
  - **`rollupTopStatus` deliberately does NOT cascade.** A roll-up is *derived* from one child's change, not an instruction — pushing it back down would flip that child's siblings whenever anyone submitted a single task for review.
  - **Descendants cascaded off `pending_review` also get their unread `approval_request` notifications marked read** (`clearTaskApprovalRequests` per cascaded id) — the parent's change just decided those reviews, so leaving them in the admin tray would point at tasks nobody is waiting on.
  - Setting a MID-level task also rolls the top up via the existing roll-up, so the whole branch plus the board-level task reflect it.
...
app/(app)/projects/[id]/page.tsx — Project detail, kanban, schedule, files (REAL uploads → pm_project_media + pm-attachments bucket; drag-drop, spinner, error panel), pinned (still local-only), content, weekly reports (pm_weekly_reports), **Technical / On-Page** (tab `seo-checklist`, between Content and Reports — renders `<SeoChecklistPanel>`; visible to anyone who can see the project, editable by admin + assigned staff; see the Technical / On-Page SEO Checklist Module), **Keywords** (tab `keywords` — renders `<KeywordResearchPanel>`; same visibility rule; see the Keyword Research Module), Weekly SEO (ADMIN-ONLY tab — renders <WeeklySeoPanel>; the tab list is built from the `ProjectTab` union and appends "weekly-seo" only when isAdmin)
app/(app)/tasks/page.tsx      — All tasks (role filtered — the list is filtered by ASSIGNEE (`t.assigneeId === user.id`), NOT by project staffing, so a staff member sees a task assigned to them even in a project they're not on; the dashboard and projects page filter by `assignedStaff` instead, so an unstaffed project's task shows up here but nowhere else). Filter bar: project / member / type / priority / status dropdowns. **The TYPE filter is service-based** (`matchesTypeFilter`, 2026-08-17 — mirrors the projects page rule of 2026-07-29): "Web Dev" and "SEO" each also match a **`both`** task, and a "Web + SEO" option isolates the combined ones. It previously used strict equality with no `both` option, so a Web + SEO task — which the project page's Add Task dialog can create — fell out of BOTH filters. The four status groups (Pending Review / Pending Client Approval / Pending Article Post / Revision Required) sort OLDEST-submission-first by statusChangedAt (FIFO review queue, user request 2026-07-31); the sort runs after the filter bar so it holds within any member/project view. **Tab switcher (2026-08-18): Project Tasks · Review · Client Approval · Article Post · Revision · To Be Discussed** — the old **"Content Approval" tab was REMOVED** (user request; it listed `pm_articles` for the separate Content module, not tasks — article approval still lives on [/content](app/(app)/content/) and the project detail Content tab, which were left untouched). The five single-status tabs share ONE tab body keyed by a `tabMeta` lookup. **All five group BY PROJECT (user request 2026-08-18)** — everything waiting for the same client sits under one heading so a project is reviewed in one pass, replacing the old Overdue/Today/Upcoming date buckets that scattered a client's tasks. The `bySubmission` FIFO sort still runs first, so it holds *within* each group AND the project holding the longest-waiting task floats to the top (the `Map` insertion order does the work). The main **Project Tasks** tab is unchanged — it keeps its date groups. Same reason, `TaskGroup`'s `overdue` red-flag now excludes `pending_client_approval` as well as `to_be_discussed` (it wrongly flagged the former until 2026-08-18, despite the doc). New Task dialog: admin can quick-create a project inline ("New project" → name only; defaults type=webdev, phase=discovery, assignedStaff=[task assignee]). Uses store.addProject which now returns the new project id.
...
components/discard-guard.tsx  — `useDiscardGuard` + the portalled "Discard unsaved changes?" confirm. EVERY dismissable dialog routes its backdrop / ✕ / Cancel through it (see the Dismiss & Discard Guard module). Never wire a new dialog's backdrop straight to its close setter.
...
lib/seo-checklist.ts                     — The STANDARD Technical / On-Page checklist (14 technical + 12 on-page) + category meta. Dependency-free: shared by the seeder and the panel so they can't drift. Editing it changes what NEW projects get, not what is already recorded.
lib/seo-checklist-db.ts                  — pm_seo_checklist_items CRUD + seedStandardChecklist (refuses to double-seed). Every helper THROWS. Called directly by the panel, NOT via the store.
components/seo-checklist-panel.tsx       — The "Technical / On-Page" project tab: progress bar, per-category sections, tick (records who + when), per-item note, rename, add, admin-only delete.
scripts/seo-checklist-schema.sql         — pm_seo_checklist_items table + touch trigger + RLS (applied live 2026-08-18). See the Technical / On-Page SEO Checklist Module.
lib/keyword-types.ts                     — Keyword types + status/priority/difficulty meta + the PURE paste parser (parseKeywordPaste / parseSeoNumber / RFC-4180 splitLine). Dependency-free and unit-checked; shared by the panel and importer.
lib/keyword-db.ts                        — pm_keywords CRUD + importKeywords (skips keywords the project already has). Every helper THROWS. Called directly by the panel, NOT via the store.
components/keyword-research-panel.tsx    — The "Keywords" project tab: summary cards, sortable table, inline rank/status/priority editing, paste importer with live preview, admin-only delete.
scripts/keywords-schema.sql              — pm_keywords table + touch trigger + RLS (applied live 2026-08-18). See the Keyword Research Module.
...
- **Re-importing is safe:** `importKeywords` skips keywords the project already holds (case-insensitive) and reports how many were skipped, rather than erroring — pasting a research set that grew by ten rows should add those ten. The `(project_id, lower(keyword))` index is for that lookup and is **intentionally not unique**, so a partial batch can't abort mid-way.
- **Permissions mirror the checklist:** admin + staff assigned to the project can add, import and edit; **delete is admin-only**; everyone else who can see the project reads it. App-level via `canEdit`, with blanket `pm_allow_all` RLS (delivery data, not money — see the Owner-Only Financial Data rule).
...
- **Not built:** rank history/charting, linking a keyword to the article that targets it (the Content module's `targetKeyword` is still just free text), and CSV *file* upload (paste covers it).

### Technical / On-Page SEO Checklist Module (2026-08-18)

Every client project has a standard set of **Technical SEO** and **On-Page SEO** work that must be done *and recorded*. Surfaced as a **"Technical / On-Page" tab** on the project detail page (between Content and Reports). Migration [scripts/seo-checklist-schema.sql](scripts/seo-checklist-schema.sql) — **applied to the LIVE project (`tfhzuruaaymfhqmeiusr`) on 2026-08-18** via the `mcp__supabase__*` MCP (identity-verified first: `get_project_url` + a live fingerprint probe — 27 article slots / 9 weekly-SEO plans / `discussion_note` present / "Al Hazima" found). Re-run on any fresh project.

- **Each project OWNS its rows — there is no shared master list.** The standard list lives in [lib/seo-checklist.ts](lib/seo-checklist.ts) (dependency-free, like `lib/weekly-seo.ts`) and is **copied** into a project by `seedStandardChecklist`, after which renaming/adding/removing an item on one client never touches another. Editing that file therefore changes what NEW projects get, **not** what is already recorded. 14 technical + 12 on-page items ship as the default.
- **Seeding is EXPLICIT, never automatic.** The empty state offers an "Add the standard checklist" button; opening a project never silently writes rows (25 live projects × 26 items would otherwise appear unbidden). `seedStandardChecklist` also refuses when the project already has items, so it cannot double-seed.
- **"Recorded" is the point:** ticking an item stamps `completed_by` + `completed_at` and the row renders "\<name\> · \<date, time\>" (date **and** time, per the app-wide timestamp rule). Unticking clears BOTH so a stale name can never sit against an open item. Each item also carries a free-text **note** for what was done / where / any reference.
- **`category` is UNCONSTRAINED text on purpose** — the `pm_tasks` status CHECK has needed a live migration for every new value, and an SEO vocabulary grows (off-page, local, …). The TS union in [lib/seo-checklist.ts](lib/seo-checklist.ts) is the source of truth and `categoryMeta()` falls back to rendering an unknown category in its own section rather than crashing. Keep that property if you add categories.
- **Permissions (user decision 2026-08-18): admin + staff assigned to the project** may tick, note, rename and add; **delete is admin-only**; everyone else who can see the project sees it read-only. Enforcement is **app-level** (`canEdit` in [app/(app)/projects/[id]/page.tsx](app/(app)/projects/[id]/page.tsx)) with blanket `pm_allow_all` RLS — matching `pm_tasks`/`pm_project_media`. This is delivery data, **not** money or credentials, so it deliberately does NOT use `pm_is_admin()`; see the Owner-Only Financial Data rule for what must.
- **NOT wired to tasks (user decision 2026-08-18).** Items are a self-contained record and do not create task rows — ~26 items per project would have swamped every board. If that changes, link with a nullable `task_id` rather than duplicating titles.
- **DB layer** [lib/seo-checklist-db.ts](lib/seo-checklist-db.ts) is called directly by the panel (invoices/expenses pattern), NOT via the Zustand store — the checklist is tab-local and doesn't belong in the global project payload. **Every helper throws** (Known Recurring Mistake #13).
- **Lint trap:** the panel must not call `setLoading(true)` inside its load effect — this repo's `react-hooks/set-state-in-effect` rule rejects it (same note as the Dismiss & Discard Guard module). `loading` starts `true` from `useState`.
- **Verified live 2026-08-18:** insert → tick (who/when recorded) → note → the `_touch` trigger moving `updated_at` → delete, then the probe row removed (table left at 0 rows). RLS proven with a real staff uid: staff can read the checklist, still see 25 projects, and **still see 0 invoices**.
