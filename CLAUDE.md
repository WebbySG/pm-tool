...
| `pm_seo_checklist_items` | ⚠ **DEPRECATED 2026-08-19, nothing reads it** — the tick-list was replaced by the SEO Setup phase TASKS (see that module). Still present live with 52 rows across 2 projects (0 ticked, 0 notes). Drop with [scripts/drop-seo-checklist.sql](scripts/drop-seo-checklist.sql) when the owner says so. |
...
app/(app)/projects/[id]/page.tsx — Project detail, kanban, schedule, files (REAL uploads → pm_project_media + pm-attachments bucket; drag-drop, spinner, error panel), pinned (still local-only), content, weekly reports (pm_weekly_reports), **SEO Work** (tab `seo-work` — renders `<SeoWorkPanel>`, the record of the standard SEO phase tasks; see the SEO Setup Phase Tasks module), **Keywords** (tab `keywords` — renders `<KeywordResearchPanel>`; see the Keyword Research Module). Both SEO tabs render only when `showSeoTabs` (project labelled seo/both, or still holding SEO work). Weekly SEO (ADMIN-ONLY tab — renders <WeeklySeoPanel>; the tab list is built from the `ProjectTab` union and appends "weekly-seo" only when isAdmin)
...
lib/seo-setup.ts                         — The STANDARD SEO work set: the "SEO Setup" parent + the three phases (keyword research / technical / on-page) and the description each task is seeded with. Dependency-free: shared by store.ensureSeoSetupTasks and the panel so they cannot drift. Editing it changes what NEW projects get, not what is already on a board.
components/seo-work-panel.tsx            — The "SEO Work" project tab: the three phases in order with status, owner, since-when, the write-up recorded in each task, and a keyword preview. Reads the store task tree; admin-only create/repair.
scripts/seo-setup-phase.sql              — pm_tasks.seo_phase + the (project_id, seo_phase) unique index (applied live 2026-08-19). See the SEO Setup Phase Tasks module.
scripts/drop-seo-checklist.sql           — Drops the deprecated pm_seo_checklist_items table. **NOT applied** — waiting on the owner.
scripts/seo-checklist-schema.sql         ⚠ DEPRECATED (2026-08-19): the tick-list table, superseded by the phase tasks. Kept for reference / rollback.
...
- **Existing SEO projects are NOT backfilled.** 14 live projects are labelled seo/both; auto-creation fires on the labelling ACTION, so those keep an empty SEO Work tab until an admin presses **Create SEO setup tasks** there (or re-saves the project type). Backfilling would have dropped 56 tasks onto live boards unasked.
- **`pm_seo_checklist_items` still EXISTS live** (52 rows, 2 projects, nothing ticked) and nothing reads it. [scripts/seo-checklist-schema.sql](scripts/seo-checklist-schema.sql) now carries a DEPRECATED header, and [scripts/drop-seo-checklist.sql](scripts/drop-seo-checklist.sql) is written but **NOT applied** — dropping a live table is the owner call.
...
- **Not built:** per-item granularity inside a phase (deliberate), per-phase due-date scheduling, and a dashboard roll-up of which clients still have unfinished SEO setup.
