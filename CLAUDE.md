...
scripts/backfill-seo-setup-tasks.mjs     — One-off: gives every live SEO project the standard work set. Idempotent, dry run by default, reads the phase text out of lib/seo-setup.ts. RUN live 2026-08-19 (56 tasks / 14 projects).
...
scripts/drop-seo-os-schema.sql — Drops the 140 dead Webby SEO OS tables, keeping the 36 pm_* plus staff_members and user_roles. Guarded by a count assertion. APPLIED live 2026-09-02.
```
...
- **Assignee (two-step rule, shared by the app and the backfill script): the project weekly SEO plan assignee if it is enrolled** (that IS the person doing this client SEO), else **the project staff member when there is exactly one**, else unassigned for the admin to tag. An unassigned task is still staff-editable — `canEdit` treats unassigned as open (see the Uploads & File Persistence module).
...
- **Existing SEO projects WERE backfilled 2026-08-19** (owner asked for the tabs to hold the tasks): [scripts/backfill-seo-setup-tasks.mjs](scripts/backfill-seo-setup-tasks.mjs) created **56 tasks across all 14 live seo/both projects** — 11 sets landed on a person (9 off their weekly SEO plan, 2 off project staffing), 3 stayed unassigned. The script is idempotent (skips any project that already holds a `seo_phase` task), reads its titles/descriptions out of `lib/seo-setup.ts` so a backfilled project matches an app-created one exactly, and defaults to a dry run (`--apply` to write). Undo, before anyone edits them: `delete from pm_tasks where seo_phase is not null;`.
