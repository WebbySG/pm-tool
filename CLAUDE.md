...
- **Revision cascade (parent → descendants, user rule 2026-08-18):** putting a task into `revision_required` marks **EVERY** descendant `revision_required` (`cascadeDescendantsRevision`, one bulk `dbUpdateTasksBulk`). Wired into all three write paths — `updateTaskStatus` (drawer status dropdown, kanban drag), `updateSubtaskStatus` (drawer subtask cycle) and `rejectTask` (the admin **Request Revision** button) — so it can't fire inconsistently. **Deliberately BROADER than the completion cascade:** that one protects `done`/`missed`/`rejected`/`pending_article_post` descendants because closing a parent must never silently discard a record, whereas revision is the opposite instruction — the work under this task has to be redone — so those are reopened too. Only descendants already in `revision_required` are skipped. Setting a MID-level task to revision also rolls the top up to `revision_required` via the existing roll-up, so the whole branch plus the board-level task reflect it.
